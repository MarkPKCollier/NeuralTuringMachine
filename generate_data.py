    ...
    def generate_batches(self, num_batches, batch_size, bits_per_vector=8, curriculum_point=20, max_seq_len=20,
        ...
        for i in range(num_batches):
            if curriculum == 'deterministic_uniform':
                ...
                seq_len = curriculum_point if np.random.random_sample() < 0.9 else np.random.randint(low=1,
                                                                                                     high=curriculum_point + 1)
            elif curriculum == 'look_back_and_forward':
                seq_len = curriculum_point if np.random.random_sample() < 0.8 else np.random.randint(low=1,
                                                                                                     high=max_seq_len + 1)
    ...
    def generate_batches(self, num_batches, batch_size, bits_per_vector=8, curriculum_point=20, max_seq_len=20,
        ...
        for i in range(num_batches):
            if curriculum == 'deterministic_uniform':
                ...
                seq_len = curriculum_point[0] if np.random.random_sample() < 0.9 else np.random.randint(low=1, high=
                curriculum_point[0] + 1)
                num_repeats = curriculum_point[1] if np.random.random_sample() < 0.9 else np.random.randint(low=1, high=
                curriculum_point[1] + 1)
            elif curriculum == 'look_back_and_forward':
                seq_len = curriculum_point[0] if np.random.random_sample() < 0.8 else np.random.randint(low=1,
                                                                                                        high=self.max_seq_len + 1)
                num_repeats = curriculum_point[1] if np.random.random_sample() < 0.8 else np.random.randint(low=1,
                                                                                                            high=self.max_repeats + 1)
            ...
            def generate_sequence():
                return np.asarray(
                    [snap_boolean(np.append(np.random.rand(bits_per_vector), (0, 0))) for _ in range(seq_len)] \
    ...
    def generate_batches(self, num_batches, batch_size, bits_per_vector=6, curriculum_point=6, max_seq_len=6,
        ...
        for i in range(num_batches):
            if curriculum == 'deterministic_uniform':
                ...
                seq_len = curriculum_point if np.random.random_sample() < 0.9 else np.random.randint(low=2,
                                                                                                     high=curriculum_point + 1)
            elif curriculum == 'look_back_and_forward':
                seq_len = curriculum_point if np.random.random_sample() < 0.8 else np.random.randint(low=2,
                                                                                                     high=max_seq_len + 1)
            ...
            def generate_item(seq_len):
                items = [
                    [snap_boolean(np.append(np.random.rand(bits_per_vector), 0)) for _ in range(NUM_VECTORS_PER_ITEM)]
            ...
            if pad_to_max_seq_len:
                full_inputs = np.concatenate(batch_inputs, eos, batch_queries, eos,
                                             np.zeros([batch_size, pad_to_len - seq_len, bits_per_vector + 1],
                                                      output_inputs), axis=1)
            else:
    ...
    def generate_batches(self, num_batches, batch_size, curriculum_point=1, curriculum='uniform',
        ...
        for i in range(num_batches):
            if curriculum == 'deterministic_uniform':
                ...
                lesson = curriculum_point if np.random.random_sample() < 0.9 else np.random.randint(low=1,
                                                                                                    high=curriculum_point + 1)
            elif curriculum == 'look_back_and_forward':
                lesson = curriculum_point if np.random.random_sample() < 0.8 else np.random.randint(low=1,
                                                                                                    high=self.num_lessons + 1)
